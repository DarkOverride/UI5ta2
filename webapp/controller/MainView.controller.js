	...
	var PageController = Controller.extend("STC.controller.MainView", {
		...
		handleValueHelpLgort: function(oEvent) {
			...
			if (!this._valueHelpDialogLgort) {
				this._valueHelpDialogLgort = sap.ui.xmlfragment(
					"STC.fragment.WerkOrtDialog",
					this
				);
			...
			// create a filter for the binding
			this._valueHelpDialogLgort.getBinding("items").filter([new Filter(
				"Lgort",
				sap.ui.model.FilterOperator.Contains, "'" + sInputValue + "'"
			)]);
		...
		handleValueHelpLgnum: function(oEvent) {
			...
			if (!this._valueHelpDialogLgnum) {
				this._valueHelpDialogLgnum = sap.ui.xmlfragment(
					"STC.fragment.WerkOrtLgnumDialog",
					this
				);
